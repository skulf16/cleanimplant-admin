...
export default function DoctorForm({ doctor, categories }: Props) {
  const isEdit = !!doctor;
  ...
  const hasDomain = (d: string) =>
    doctor?.domains?.includes(d as "DE" | "COM") ?? d === "DE";

  const hasCategory = (id: string) =>
    doctor?.categories?.some((c: { category: { id: string; name: string } }) => c.category.id === id) ?? false;
    ...
    <form action={formAction} className="space-y-8 max-w-3xl">
      ...
      {categories.length > 0 && (
        <fieldset className="bg-white border border-[#e5e7eb] rounded-[6px] p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
            {categories.map((cat) => (
              <label key={cat.id} className="flex items-center gap-2 text-[13px] text-[#555] cursor-pointer">
                <input
                  type="checkbox"
                  name="categoryIds"
                  value={cat.id}
                  defaultChecked={hasCategory(cat.id)}
                  className="rounded border-gray-300 text-[#30A2F1] focus:ring-[#30A2F1]"
                />
                {cat.name}
              </label>
      ...
      <fieldset className="bg-white border border-[#e5e7eb] rounded-[6px] p-6">
        <legend className="text-[13px] font-semibold text-[#555] tracking-wide px-2 mb-4">
          Veröffentlichung & Domains
        </legend>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-3">
            <label className="flex items-center gap-2.5 cursor-pointer">
              <input type="checkbox" name="active" defaultChecked={doctor?.active ?? true} className="rounded" />
              <span className="text-[14px] text-[#333]">Aktiv (öffentlich sichtbar)</span>
            </label>
            <label className="flex items-center gap-2.5 cursor-pointer">
              <input type="checkbox" name="verified" defaultChecked={doctor?.verified ?? false} className="rounded" />
              <span className="text-[14px] text-[#333]">Verifiziert</span>
            </label>
            <label className="flex items-center gap-2.5 cursor-pointer">
              <input type="checkbox" name="featured" defaultChecked={doctor?.featured ?? false} className="rounded" />
              <span className="text-[14px] text-[#333]">Empfohlen (Startseite)</span>
            </label>
          </div>
          <div>
            <p className="text-[13px] font-medium text-[#555] mb-2">Domain-Zuweisung</p>
            <div className="space-y-2">
              <label className="flex items-center gap-2.5 cursor-pointer">
                <input type="checkbox" name="domains" value="DE" defaultChecked={hasDomain("DE")} className="rounded" />
                <span className="text-[14px] text-[#333]">mycleandent<strong>.de</strong> (DACH)</span>
              </label>
              <label className="flex items-center gap-2.5 cursor-pointer">
                <input type="checkbox" name="domains" value="COM" defaultChecked={hasDomain("COM")} className="rounded" />
                <span className="text-[14px] text-[#333]">mycleandent<strong>.com</strong> (International)</span>
              </label>
            </div>
          </div>
        </div>
      ...
      <fieldset className="bg-white border border-[#e5e7eb] rounded-[6px] p-6">
        ...
        <div className="space-y-4">
          <div>
            ...
            <input name="metaTitle" defaultValue={val("metaTitle")} className="input" />
          </div>
          <div>
            <label className="label">Meta-Beschreibung</label>
            <textarea name="metaDesc" defaultValue={val("metaDesc")} rows={2} className="input resize-none" />
          </div>
        </div>
      </fieldset>
      ...
      <div className="flex items-center gap-4 pb-8">
        ...
        >
          {pending ? "Speichern…" : isEdit ? "Änderungen speichern" : "Profil anlegen"}
        </button>
