    ...
    <form ref={formRef} onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-lg p-6 space-y-5">
          ...
          >
            {uploading ? (
              ...
              <>
                ...
                <span className="text-[12px] text-gray-400">Datei hier ablegen oder klicken zum Auswählen</span>
                <span className="text-[11px] text-gray-300">PNG, SVG, PDF, ZIP, MP4 …</span>
              </>
            )}
          </div>
        )}
        <input ref={inputRef} type="file" className="hidden" onChange={e => { const f = e.target.files?.[0]; if (f) uploadFile(f); e.target.value = ""; }} />
      </div>

      {/* Titel */}
      <div>
        <label className="label">Titel *</label>
        <input name="title" type="text" required className="input w-full" placeholder="z. B. mycleandent Logo PNG" />
      </div>

      {/* Beschreibung */}
      <div>
        <label className="label">Beschreibung (optional)</label>
        <textarea name="description" rows={2} className="input w-full resize-none" placeholder="Kurze Beschreibung der Datei …" />
      </div>

      {/* Download-Link */}
      <div>
        <label className="label">Download-Link (optional)</label>
        <input name="downloadUrl" type="url" className="input w-full" placeholder="https://dropbox.com/… oder WeTransfer-Link" />
        <p className="text-[11px] text-[#999] mt-1">Alternativer Download-Link z. B. für große Dateien via Dropbox oder WeTransfer.</p>
      </div>
      ...
      <div>
        ...
        <select name="category" required className="input w-full">
          ...
          <option value="digital">Digitaler Content</option>
